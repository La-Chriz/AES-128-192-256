...
void InvSubState(uint8_t *state)
{
  ...
  state[1]  =  ibox[state[1]];
  state[2]  =  ibox[state[2]];
  state[3]  =  ibox[state[3]];
  state[4]  =  ibox[state[4]];
  state[5]  =  ibox[state[5]];
  state[6]  =  ibox[state[6]];
  state[7]  =  ibox[state[7]];
  state[8]  =  ibox[state[8]];
  state[9]  =  ibox[state[9]];
  state[10] = ibox[state[10]];
  state[11] = ibox[state[11]];
  state[12] = ibox[state[12]];
  state[13] = ibox[state[13]];
  state[14] = ibox[state[14]];
  state[15] = ibox[state[15]];
}
...
static void InvMixColumns(uint8_t *state)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    state[i * 4 + 0] = gmul_14[a] ^
                       gmul_11[b] ^
                       gmul_13[c] ^
                       gmul_09[d];

    state[i * 4 + 1] = gmul_09[a] ^
                       gmul_14[b] ^
                       gmul_11[c] ^
                       gmul_13[d];

    state[i * 4 + 2] = gmul_13[a] ^
                       gmul_09[b] ^
                       gmul_14[c] ^
                       gmul_11[d];

    state[i * 4 + 3] = gmul_11[a] ^
                       gmul_13[b] ^
                       gmul_09[c] ^
                       gmul_14[d];
  }
...
static void AddRoundKey(uint8_t *state, const uint8_t *rk)
{
  state[0]  ^=  rk[0];
  state[1]  ^=  rk[1];
  state[2]  ^=  rk[2];
